...
```{r}
# Clean and split dimensions (remove " cm")
raw <- raw %>%
  mutate(
...
```{r}
# -------------------------------
# 4. Create Category-Level Time Series (FIXED)
# -------------------------------
cat_daily <- raw %>%
...
# Ensure at least 60 points exist; otherwise reduce horizon
...
```{r}
# -------------------------------
# 10. Hierarchical Reconciliation (Optional)
# -------------------------------
...
# Ensure daily_totals exists (from earlier steps)
# daily_totals contains: stock_xfer_date, total_stock_qty, total_space_m3
...
# ---------------------------------------------------------
# 4. Optional: Display scatterplot with log scaling
#    (useful if distribution is wide)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# 2. Summary statistics per category (space + stock)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# 6. Plot: Daily Category Space Usage (m³)
# ---------------------------------------------------------
...
############################################################
#   FORECASTED RESULTS FOR VAR MODEL (CATEGORY SPACE)
############################################################
...
# Extract forecasts into matrix
fc_matrix <- sapply(fc$fcst, function(x) x[,1])

# Convert differenced forecast → level forecast
last_values <- tail(ts_cat_space, 1)
level_fc <- apply(fc_matrix, 2, function(diff_series) {
  cumsum(c(last_values, diff_series))[-1]
})
```

```{r}
par(mfrow = c(2, 2))
for (i in 1:ncol(level_fc)) {
  plot(level_fc[,i], type="l", col="blue",
       main=paste("VAR 30-Day Forecast –", colnames(ts_cat_space)[i]),
       ylab="Space (m3)", xlab="Forecast Horizon")
}
par(mfrow = c(1, 1))
...
```{r}
irf_res <- irf(var_model, impulse = NULL, response = NULL,
...
residual_matrix <- do.call(cbind, lapply(var_res, function(x) x[,1]))

library(corrplot)
corrplot(cor(residual_matrix), method="color",
         title="Residual Cross-Correlation", mar=c(0,0,1,0))
